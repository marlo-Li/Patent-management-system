...
class CheckApplicantFrame(wx.Frame):
    def __init__(self, parent, title, select_id, dbhelper):
        ...
        # tc_degreeType = wx.TextCtrl(self.panel, -1)  # 学历/学位
    ...
    def showAllText(self):
        ...
        self.phone.SetValue(str(data[5]))  # 设置手机号
        self.QQ.SetValue(str(data[6]))  # 设置QQ
        self.email.SetValue(str(data[7]))  # 设置电子邮箱
        self.checkstate.SetValue(str(data[8]))  # 设置用户状态

    # 修改申请人信息并保存
    def saveUpdate(self, evt):
        '''保存修改后的值'''
        sql = "update user_info set UserStatus=%s where IDCard=%s"
        userstate = self.checkstate.GetValue()
        try:
            param = (userstate, self.id)
            self.dbhelper.UpdateData(sql, param)
        except Exception as e:
            print("SQL执行错误，原因：", e)

        # 插入数据库
        # self.checkUserUpdate(self.id, state)  # id就是身份证号
        # 插入列表
        print(self.select_id, "user_state is:", userstate)
        self.mainframe.list.SetItem(self.select_id, 7, userstate)
        self.Destroy()  # 销毁隐藏Dialog

    # 管理员审核用户信息时使用,通过申请人编号查询申请人信息
    def getApplicantById(self, id):
        '''根据申请人id值来寻找申请人信息'''
        sql = "select Password, TrueName, IDCard, Gender, Birthday, Phone,QQ, Email, UserStatus " \
                  "from user_info " \
              "where IDCard=%s"
        try:
            param = (id,)
...
class AdminApplicant(wx.Panel):
    def __init__(self, notebook, parent, dbhelper):
        ...
        self.list.InsertColumn(4, "手机号")
        self.list.InsertColumn(5, "QQ号")
        self.list.InsertColumn(6, "电子邮箱")
        self.list.InsertColumn(7, "审核状态")
        ...
        self.list.SetColumnWidth(5, 80)
        self.list.SetColumnWidth(6, 140)
        self.list.SetColumnWidth(7, 110)
        #  设置字体
    ...
    def AdminPagingQueryUser(self, startRow, pageSize):
        sql = "select TrueName, IDCard, Gender,Birthday,Phone,QQ, Email,UserStatus " \
                  "from user_info " \
              "order by IDCard offset %s rows fetch next %s rows only"
        try:
    ...
    def AdmingetApplicantByValue(self, label_value, label_value2, startRow, pageSize):
        ...
        if label_value2 == '身份证号':
            sql = "select TrueName, IDCard, Gender,Birthday,Phone,QQ, Email,UserStatus " \
                  "from user_info " \
                  "where IDCard like CONCAT('%%',%s,'%%') order by IDCard offset %s rows fetch next %s rows only"
        elif label_value2 == '电话':
            sql = "select TrueName, IDCard, Gender,Birthday,Phone,QQ, Email,UserStatus " \
                  "from user_info " \
                  "where Phone like CONCAT('%%',%s,'%%') order by IDCard offset %s rows fetch next %s rows only"
        elif label_value2 == '姓名':
            sql = "select TrueName, IDCard, Gender,Birthday,Phone,QQ, Email,UserStatus " \
                  "from user_info " \
                  "where TrueName like CONCAT('%%',%s,'%%') order by IDCard offset %s rows fetch next %s rows only"
        elif label_value2 == 'QQ':
            sql = "select TrueName, IDCard, Gender,Birthday,Phone,QQ, Email,UserStatus " \
                  "from user_info " \
                  "where QQ like CONCAT('%%',%s,'%%') order by IDCard offset %s rows fetch next %s rows only"
        try:
