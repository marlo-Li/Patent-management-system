...
class AddPatentFrame(wx.Frame):
    def __init__(self, parent, id, name, title, dbhelper):
        ...
        baseclass_text = wx.TextCtrl(self.panel, style=wx.TE_MULTILINE)
        areaclass_text = wx.TextCtrl(self.panel, style=wx.TE_MULTILINE)
        funclass_text = wx.TextCtrl(self.panel, style=wx.TE_MULTILINE)
    ...
    def savePatent(self, evt):
        ...
        baseclass = self.BaseClass.GetValue()
        areaclass = self.RegionalCategory.GetValue()
        funclass = self.FunctionCategory.GetValue()
        ...
        if patentName == "" or fun_info == "" or baseclass == None or areaclass == None or funclass == None:
            ...
            # 对Patent进行实例化
            patent = Patent('', PatentName=patentName, PatentFunctional=fun_info, BaseClass=baseclass,
                            RegionalCategory=areaclass, FunctionCategory=funclass, ApplicantName=appName,
                            ApplicantID=appID, ModifyTime=appdate, SubmitTime='', AuditTime='', patentstate='待审核')
            # 判断专利名称是否重复
            try:
                param_1 = (patentName,)
                ...
                if i:
                    ...
                    # 进行数据库添加
                    try:
                        coun = str(self.dbhelper.Count("SELECT COUNT(*) from patent_info")[0][0]+1)
                        param_2 = (coun, patent.getPatentName(), patent.getPatentFunctional(),
                                   patent.getBaseClass(), patent.getRegionalCategory(), patent.getFunctionCategory(),
                                   patent.getApplicantID(), patent.getApplicationDate(), '待审核')
                        sql_2 = "insert into patent_info(Patentid, PatentName, PatentFunctional, BaseClass, RegionalCategory, FunctionCategory,\
                                       ApplicantID, ModifyTime, patentstate) values(%s,%s,%s,%s,%s,%s,%s,%s,%s)"
                        ...
                        # 再进行列表添加
                        self.addToList(patentid, '待审核')  # 加一个专利状态变量，设为保存，修改这个函数
...
class UpdatePatentFrame(wx.Frame):
    ...
    def __init__(self, parent, title, select_id, dbhelper):
        ...
        patentName_text = wx.TextCtrl(self.panel)
        fun_info_text = wx.TextCtrl(self.panel, style=wx.TE_MULTILINE)
        baseclass_text = wx.ComboBox(self.panel, style=wx.TE_MULTILINE)
        areaclass_text = wx.ComboBox(self.panel, style=wx.TE_MULTILINE)
        funclass_text = wx.ComboBox(self.panel, style=wx.TE_MULTILINE)
    ...
    def saveUpdate(self, evt):
        ...
        appName = self.ApplicantName.GetValue()
        appID = self.ApplicantID.GetValue()
        patentstate = self.patentstatus.GetValue()

        if patentstate == '待审核' or patentstate == '未审核' or patentstate == '审核未通过':
        ...
        if patentName == "" or fun_info == "" or baseclass == "" or areaclass == "" or funclass == "" or patentstate == "":
            ...
            patent = Patent(Patentid=patentID, PatentName=patentName, PatentFunctional=fun_info, BaseClass=baseclass,
                            RegionalCategory=areaclass, FunctionCategory=funclass, ApplicantName=appName,
                            ApplicantID=appID, ModifyTime=self.change_date,
                            SubmitTime=self.submit_date)  # 将数据封装到patent对象中
            try:
                param = (patent.getPatentName(), patent.getPatentFunctional(),
                         patent.getBaseClass(), patent.getRegionalCategory(), patent.getFunctionCategory(),
                         patent.getPassTime(), self.patentid)
                print(param)
                sql_2 = "update patent_info set PatentName=%s, PatentFunctional=%s, BaseClass=%s, RegionalCategory=%s, " \
                        "FunctionCategory=%s, SubmitTime=%s where patent_info.Patentid=%s"
    ...
    def getPatentInfo(self, patentid):
        param = (patentid,)
        sql_1 = "select Patentid, PatentName, PatentFunctional, BaseClass, RegionalCategory, FunctionCategory," \
                "user_info.TrueName, ApplicantID, ModifyTime, patentstate " \
                "from patent_info " \
                "INNER JOIN user_info ON patent_info.ApplicantID=user_info.IDCard " \
                "where patent_info.Patentid=%s"
        try:
            data = self.dbhelper.SelectRecord(sql_1, param)  # 通过专利编号获取专利信息
            return data
        except Exception as e:
            print("查询记录失败:", e)
...
class OneManPatent(wx.Panel):
    ...
    def showPatent(self, label_value='', label_value3='', label_value1='', label_value2=''):
        # label_value:根据输入的东西的关键字，label_value1:起始申请日，label_value2:截止申请日，label_value3:类型
        '''显示原始信息'''
    ...
    def queryPatent(self, evt):
        ...
        if str(label_value1) != 'INVALID DateTime' and str(label_value2) != 'INVALID DateTime':
            ...
            start_time = None
            deadline = None
        # 判断 label_value3是选择框，label_value是关键词框
        if label_value3 != '未选择':
            # 关键词为空的
    ...
    def UserGetPatentByValue(self, label_value, label_value2, label_value3, label_value4, eid):
        """label_value关键字, label_value2类型, label_value3起始日期, label_value4截止日期"""
        # 定义SQL语句
        sql = ""
        if label_value2 == '专利名称':
            if label_value3 == None:
                ...
                sql = "select PatentID, PatentName, BaseClass, RegionalCategory, FunctionCategory, " \
                      "user_info.TrueName, AuditTime, patentstate " \
                      "from patent_info " \
                      "INNER JOIN user_info ON patent_info.ApplicantID=user_info.IDCard " \
                      "where patent_info.PatentName like CONCAT('%%',%s,'%%') and patent_info.ApplicantID=%s "\
                      "or (AuditTime between %s and %s)"
            else:
                sql = "select PatentID, PatentName, BaseClass, RegionalCategory, FunctionCategory, " \
                      "user_info.TrueName, AuditTime, patentstate " \
                      "from patent_info " \
            ...
            if label_value3 == None:
                sql = "select PatentID, PatentName, BaseClass, RegionalCategory, FunctionCategory, " \
                      ...
                      "from patent_info " \
                      "INNER JOIN user_info ON patent_info.ApplicantID=user_info.IDCard " \
                      "where RegionalCategory like CONCAT('%%',%s,'%%') and patent_info.ApplicantID=%s " \
                      "or (AuditTime between %s and %s)"
            else:
                sql = "select PatentID, PatentName, BaseClass, RegionalCategory, FunctionCategory, " \
                      "user_info.TrueName, AuditTime, patentstate " \
            ...
            if label_value3 == None:
                sql = "select PatentID, PatentName, BaseClass, RegionalCategory, FunctionCategory, " \
                      ...
                      "from patent_info " \
                      "INNER JOIN user_info ON patent_info.ApplicantID=user_info.IDCard " \
                      "where FunctionCategory like CONCAT('%%',%s,'%%') and patent_info.ApplicantID=%s " \
                      "or (AuditTime between %s and %s)"
            else:
                sql = "select PatentID, PatentName, BaseClass, RegionalCategory, FunctionCategory, " \
                      "user_info.TrueName, AuditTime, patentstate " \
            ...
            if label_value3 == None:
                sql = "select PatentID, PatentName, BaseClass, RegionalCategory, FunctionCategory, " \
                      ...
                      "from patent_info " \
                      "INNER JOIN user_info ON patent_info.ApplicantID=user_info.IDCard " \
                      "where BaseClass like CONCAT('%%',%s,'%%') and patent_info.ApplicantID=%s " \
                      "or (AuditTime between %s and %s)"
            else:
                sql = "select PatentID, PatentName, BaseClass, RegionalCategory, FunctionCategory, " \
                      "user_info.TrueName, AuditTime, patentstate " \
            ...
            if label_value3 == None:
                sql = "select PatentID, PatentName, BaseClass, RegionalCategory, FunctionCategory, " \
                      ...
                      "from patent_info " \
                      "INNER JOIN user_info ON patent_info.ApplicantID=user_info.IDCard " \
                      "where user_info.UserStatus like CONCAT('%%',%s,'%%') and patent_info.ApplicantID=%s " \
                      "or (AuditTime between %s and %s)"
            else:
                sql = "select PatentID, PatentName, BaseClass, RegionalCategory, FunctionCategory, " \
                      "user_info.TrueName, AuditTime, patentstate " \
            ...
            sql = "select PatentID, PatentName, BaseClass, RegionalCategory, FunctionCategory, " \
                  ...
                  "from patent_info " \
                  "INNER JOIN user_info ON patent_info.ApplicantID=user_info.IDCard " \
                  "where user_info.UserStatus like CONCAT('%%',%s,'%%') and patent_info.ApplicantID=%s and (AuditTime between %s and %s)"
        try:
            param = (label_value, eid, label_value3, label_value4)
            # param = (label_value, eid)
    ...
    def getAllPatentByeID(self):
        # sql = "select PatentID, PatentName, base_category.BaseName, region_category.RegionName, " \
        #       "function_category.FunctionName, user_info.TrueName, AuditDate, patent_state.StateName " \
        #       "INNER JOIN base_category ON patent_info.BaseCategory=base_category.id  " \
        #       "INNER JOIN region_category ON patent_info.RegionCategory=region_category.id " \
        #       "INNER JOIN function_category ON patent_info.FunctionCategory=function_category.id   where patent_info.IDCard=%s"
        sql = "select Patentid, PatentName, BaseClass, RegionalCategory, FunctionCategory, " \
              "user_info.TrueName, AuditTime, " \
              "patentstate from patent_info " \
              "INNER JOIN user_info ON patent_info.ApplicantID=user_info.IDCard where patent_info.ApplicantID=%s"
        try:
            param = (self.Eid,)
            datas = self.dbhelper.SelectRecord(sql, param)
            return datas
