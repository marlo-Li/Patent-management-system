...
class CheckPatentFrame(wx.Frame):
    ...
    def saveUpdate(self, evt):
        ...
        try:
            ...
            sql = "update patent_info set patentstate=%s, AuditTime=%s where patentid=%s"
    ...
    def AdminGetPatentById(self, patentid):
        '''根据申请人id值来寻找专利信息'''
        # 定义SQL语句
        sql = "select PatentID, PatentName, PatentFunctional, BaseClass, RegionalCategory, FunctionCategory,\
                                      user_info.TrueName, patent_info.ApplicantID, SubmitTime, patentstate \
                               from patent_info \
...
class AdminPatent(wx.Panel):
    ...
    def showPatent(self, label_value='', label_value3='', label_value1='', label_value2='', datas=None):
        ...
        if datas != None:
            for data in datas:
                ...
                self.list.SetItem(index, 2, str(data[3]))
                self.list.SetItem(index, 3, str(data[4]))
                self.list.SetItem(index, 4, str(data[5]))
                self.list.SetItem(index, 5, str(data[6]))
                self.list.SetItem(index, 6, str(data[8]))
                self.list.SetItem(index, 7, str(data[9]))
    ...
    def AdminPagingQuery(self, start_time, end_time, startRow, pageSize):
        sql = "select PatentID, PatentName, PatentFunctional, BaseClass, RegionalCategory, FunctionCategory,\
                               ...
                               from patent_info \
                                    INNER JOIN user_info ON patent_info.ApplicantID=user_info.IDCard \
                                where AuditTime between %s and %s order by Patentid offset %s rows fetch next %s rows only"
        try:
        ...
        if label_value2 == '身份证号':
            if label_value3 == None:
                sql = "select PatentID, PatentName, PatentFunctional, BaseClass, RegionalCategory, FunctionCategory,\
                               ...
                               from patent_info \
                                    INNER JOIN user_info ON patent_info.ApplicantID=user_info.IDCard \
                                where patent_info.ApplicantID like CONCAT('%%',%s,'%%')\
                                     or (AuditTime between %s and %s) order by Patentid offset %s rows fetch next %s rows only"
            else:
                sql = "select PatentID, PatentName, PatentFunctional, BaseClass, RegionalCategory, FunctionCategory,\
                                      user_info.TrueName, patent_info.ApplicantID, SubmitTime, patentstate \
                               from patent_info \
                                    INNER JOIN user_info ON patent_info.ApplicantID=user_info.IDCard \
                                where patent_info.ApplicantID like CONCAT('%%',%s,'%%')\
                                     and (AuditTime between %s and %s) order by Patentid offset %s rows fetch next %s rows only"
            ...
            if label_value3 == None:
                sql = "select PatentID, PatentName, PatentFunctional, BaseClass, RegionalCategory, FunctionCategory,\
                               ...
                               from patent_info \
                                    INNER JOIN user_info ON patent_info.ApplicantID=user_info.IDCard \
                                where patentstate like CONCAT('%%',%s,'%%') \
                                   or (AuditTime between %s and %s) order by Patentid offset %s rows fetch next %s rows only"
            else:
                sql = "select PatentID, PatentName, PatentFunctional, BaseClass, RegionalCategory, FunctionCategory,\
                                      user_info.TrueName, patent_info.ApplicantID, SubmitTime, patentstate \
                               from patent_info \
                                    INNER JOIN user_info ON patent_info.ApplicantID=user_info.IDCard \
                                where patentstate like CONCAT('%%',%s,'%%') and (AuditTime between %s and %s) " \
                      "order by Patentid offset %s rows fetch next %s rows only"
        try:
    ...
    def AdminGetCount(self, state=None, label_value=None, label_value2=None, label_value3=None, label_value4=None):
        ...
        if state == 2:  # 管理员查询专利信息使用
            ...
            if label_value3 == None:
                sql = "select count(*) from patent_info where ApplicantID like CONCAT('%%',%s,'%%') or (AuditTime between %s and %s)"
            else:
                sql = "select count(*) from patent_info where ApplicantID like CONCAT('%%',%s,'%%') and (AuditTime between %s and %s)"
        elif label_value2 == '专利名称':
            if label_value3 == None:
                sql = "select count(*) from patent_info where PatentName like CONCAT('%%',%s,'%%') or (AuditTime between %s and %s)"
            else:
                sql = "select count(*) from patent_info where PatentName like CONCAT('%%',%s,'%%') and (AuditTime between %s and %s)"
        elif label_value2 == '专利状态':
            if label_value3 == None:
                sql = "select count(*) from patent_info \
                            where patentstate like CONCAT('%%',%s,'%%') or (AuditTime between %s and %s)"
            else:
                sql = "select count(*) from patent_info \
                            where patentstate like CONCAT('%%',%s,'%%') and (AuditTime between %s and %s)"
        elif label_value2 == '未选择':
            sql = "select count(*) from patent_info \
                      where AuditTime between %s and %s"
        try:
            if state == 2:
                param = ()
                datas = self.dbhelper.SelectRecord(sql, param)
