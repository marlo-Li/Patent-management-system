...
class LoginInterface(wx.Panel):
    ...
    def judeg_user_type(self, user_type):
        ...
        sql_2 = "select Password,UserCategory from user_info where IDCard=%s"
        sql = "select UserStatus,TrueName from user_info where IDCard=%s"
        # 判断管理员登录
        if user_type == 0:
            try:
