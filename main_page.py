...
class mainFrame(wx.Frame):
    ...
    def __init__(self, parent, title):
        ...
        bmp = wx.Image("image/logo5.jpg", wx.BITMAP_TYPE_JPEG)
    ...
    # 读操作
    def readfile(self):
        filename = 'db\\db_connect_info.txt'
    ...
    def Userfun(self, user_state, true_name, eID, user_type):
        ...
        #
        if user_type == '用户':
            self.patentInfo = OneManPatent(self.nb, self, user_state, true_name, eID, self.dbhelper)  # 个人专利信息界面
            self.Query = UserQueryPatent(self.nb, self, self.dbhelper)  # 查询专利信息界面
            self.userInfo = User_info(self.nb, self, eID, self.dbhelper)  # 用户基本信息界面
            #
