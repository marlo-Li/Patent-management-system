...
class UserQueryPatent(wx.Panel):
    ...
    def showPatent(self, label_value='', label_value3='', label_value1='', label_value2='', datas=None):
        ...
        if label_value == '' and label_value3 == '未选择':
            ...
            if count[0][0] != 0:
                # 计算总页数
                self.totalPage = int(count[0][0] / self.pageSize)  # 返回的数量/一个页面容纳数量即页数
                ...
                wx.StaticText(self, label="第" + str(self.curPage) + "页" + "/共" + str(self.totalPage) + "页",
                              pos=(180, 510), size=(100, 25))  # 显示页面按钮
                self.startRow = (self.curPage - 1) * self.pageSize  # 起始索引
                datas = self.UserPagingQuery(label_value1, label_value2, self.startRow, self.pageSize)  # 用户通过日期分页查询所有审核通过的专利信息
            elif count[0][0] == 0:
        ...
        # 通过关键字和时间联合查询
            ...
            if count[0][0] != 0:
                ...
                self.startRow = (self.curPage - 1) * self.pageSize  # 起始索引
                print("有", self.startRow, "行", "一页有", self.pageSize, "个")
    ...
    def UserPagingQuery(self, start_time, end_time, startRow, pageSize):
        sql_1 = "select PatentID, PatentName, BaseClass, RegionalCategory, FunctionCategory,\
                                  user_info.TrueName, AuditTime, patentstate \
                           from patent_info \
                                INNER JOIN user_info ON patent_info.ApplicantID=user_info.IDCard \
                                where patentstate='审核通过' and (AuditTime between %s and %s) order by Patentid offset %s rows fetch next %s rows only"
        try:
        ...
        if label_value2 == '申请人姓名':
            if label_value3 == None:
                sql = "select PatentID, PatentName, BaseClass, RegionalCategory, FunctionCategory,\
                           ...
                           from patent_info \
                                INNER JOIN user_info ON patent_info.ApplicantID=user_info.IDCard \
                                where patentstate='审核通过' and user_info.TrueName like CONCAT('%%',%s,'%%')\
            ...
            if label_value3 == None:
                sql = "select PatentID, PatentName, BaseClass, RegionalCategory, FunctionCategory,\
                           ...
                           from patent_info \
                                INNER JOIN user_info ON patent_info.ApplicantID=user_info.IDCard \
                                where patentstate='审核通过' and patent_info.PatentName like CONCAT('%%',%s,'%%')\
                                     or (AuditTime between %s and %s) order by Patentid offset %s rows fetch next %s rows only"
            else:
                sql = "select PatentID, PatentName, BaseClass, RegionalCategory, FunctionCategory,\
                                  user_info.TrueName, AuditTime, patentstate \
    ...
    def UserGetCount(self, state=1, label_value=None, label_value2=None, label_value3=None, label_value4=None):
        ...
        if state == 1:  # 用户查询专利信息使用
            sql = "select count(*) from patent_info where patentstate='审核通过'"
        elif label_value2 == '申请人姓名':
            if label_value3 == None:
                sql = "select count(*) from patent_info " \
                      "INNER JOIN user_info ON patent_info.ApplicantID=user_info.IDCard " \
                      "where user_info.TrueName like CONCAT('%%',%s,'%%') and patentstate='审核通过' or (AuditTime between %s and %s)"
            else:
                sql = "select count(*) from patent_info " \
                      "INNER JOIN user_info ON patent_info.ApplicantID=user_info.IDCard " \
                      "where user_info.TrueName like CONCAT('%%',%s,'%%') and patentstate='审核通过' and (AuditTime between %s and %s)"
        elif label_value2 == '专利名称':
            if label_value3 == None:
                sql = "select count(*) from patent_info " \
                      "where PatentName like CONCAT('%%',%s,'%%') and patentstate='审核通过' or (AuditTime between %s and %s)"
            else:
                sql = "select count(*) from patent_info " \
                      "where PatentName like CONCAT('%%',%s,'%%') and patentstate='审核通过' and (AuditTime between %s and %s)"
        elif label_value2 == '未选择':
            sql = "select count(*) from patent_info where patentstate='审核通过' and (AuditTime between %s and %s)"
        try:
            if state == 1:
                param = ()
                datas = self.dbhelper.SelectRecord(sql, param)
