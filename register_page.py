...
class RegisterInterface(wx.Panel):
    ...
    def savePatent(self, evt):
        ...
        confirmpd = self.confirmpd.GetValue().strip()  # 确认密码
        ...
        if password != "" and name != "" and certNo != "" and judge == 1:
            ...
            user = UserInfo('', password, name, certNo, gender, birthday_time, CellPhoneNumber=shoujihao, QQ=qq, email=email)
            # 判断是否能找到相同身份证号
            try:
                param_1 = (certNo,)
                ...
                if i:
                    ...
                    try:
                        ...
                        param_2 = ("用户", user.getpassword(), user.getApplicantName(), user.getApplicantID(),
                                   user.getgender(), user.getbirthday(), user.getCellPhoneNumber(),
                                   user.getQQ(), user.getemail(), "待审核")
                        sql_2 = "insert into user_info(UserCategory,Password,TrueName,IDCard, Gender, Birthday, Phone, " \
                                "QQ, Email,UserStatus) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
